...
const MODELS = [
  { id: 'classic', name: 'Classic Nordic', desc: 'Tradycyjna, prosta konstrukcja idealna do każdego ogrodu.', w: 200, d: 200, img: 'https://bertsch-holzbau.eu/wp-content/uploads/2024/07/Lasse-2.jpg' },
  { id: 'modern', name: 'Modern Glass', desc: 'Przeszklony front, nowoczesny design zapewniający świetne widoki.', w: 250, d: 200, img: 'https://bertsch-holzbau.eu/wp-content/uploads/2024/08/001514-01-kopia-600x450.webp' },
  { id: 'premium', name: 'Premium Spa', desc: 'Duża przestrzeń i ekskluzywne wykończenie z miejscem do wypoczynku.', w: 300, d: 250, img: 'https://bertsch-holzbau.eu/wp-content/uploads/2024/07/Jarle-2.jpg' },
];
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                  {MODELS.map(model => (
                    <button 
                      ...
                      className={\`group flex flex-col text-left overflow-hidden rounded-[1.25rem] border-2 transition-all duration-300 \${config.selectedModel === model.id ? 'border-brand-green bg-brand-green/5 shadow-md transform -translate-y-1' : 'border-gray-100 bg-white hover:border-brand-green/30 hover:shadow-sm'}\`}
                    >
                      <div className="w-full h-40 bg-gray-100 relative overflow-hidden">
                        ...
                        <div className={\`absolute top-3 right-3 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors \${config.selectedModel === model.id ? 'bg-brand-green border-brand-green' : 'bg-white/50 border-white/80 backdrop-blur-sm'}\`}>
                          {config.selectedModel === model.id && <Check className="w-3 h-3 text-white" strokeWidth={3} />}
                        </div>
                      </div>
                      <div className="p-5 flex flex-col flex-grow">
                        <h4 className="font-heading font-bold text-lg text-brand-graphite mb-2">{model.name}</h4>
                        <p className="text-gray-500 text-xs mb-4 flex-grow">{model.desc}</p>
                        <div className="inline-block px-2.5 py-1 bg-white rounded-md text-[11px] font-bold text-gray-500 border border-gray-100 self-start uppercase tracking-widest shadow-sm">
                          {model.w} × {model.d} cm
                        </div>
                      </div>
                    </button>
